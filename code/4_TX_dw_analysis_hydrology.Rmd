...
# how many pwsids in East TX have health-baed violations?
viols %>%
  as.data.frame() %>%
  select(-geometry) %>%
  filter(east_tx_flag == "yes") %>%
...
etx_summary_filt <- etx_summary[order(etx_summary$n),] %>%
  filter(n>=150)
ggplot(etx_summary_filt, aes(x = reorder(value_description, -n),
                        y = n, 
                        fill = is_health_based_ind)) + 
  geom_bar(stat = "identity") + 
  scale_fill_discrete(name = "Health-based?") + 
...
# crosswalking tract data: 
ej_haz_crosswalked <- merge(water_haz, crosswalk,
                            by.x = "id", by.y = "tract_geoid")

# grabbing the weighted mean proximity to each hazard: 
ej_haz_weighted <- ej_haz_crosswalked %>%
  group_by(pwsid) %>%
  mutate(superfund_weighted = weighted.mean(superfund, tract_parcel_weight), 
         rmp_weighted = weighted.mean(rmp, tract_parcel_weight), 
         haz_waste_weighted = weighted.mean(haz_waste, tract_parcel_weight), 
         storage_tanks_weighted = weighted.mean(storage_tanks, tract_parcel_weight), 
         waste_discharge_weighted = weighted.mean(waste_discharge, tract_parcel_weight)) %>%
  select(pwsid, superfund_weighted:waste_discharge_weighted) %>%
  left_join(keys$analysis_keys) %>%
  distinct()

# what's the relationship between these distances and violations? 
ej_haz_long <- pivot_longer(ej_haz_weighted, superfund_weighted:waste_discharge_weighted)
...
ggplot(ej_haz_long, aes(x = value, 
  ...
  labs(x = "Distance to Hazard", y = "Health-based Violations, 5yr")


# what about general mean distance to a hazard? 
ej_haz_weighted <- as.data.frame(ej_haz_weighted)
...
pop_source <- demo$census %>%
  ...
  mutate(percent_pop = 
           case_when(east_tx_flag == "no" ~ (pop_served / tx_pop)*100, 
                     east_tx_flag == "yes" ~ (pop_served / etx_pop$total_pop)*100))
