...
leaflet() %>%
  ...
  addPolygons(data = top_viol,
              ...
              label = paste0("pwsid: ", top_viol$pwsid,
                "; top viol & cont: ", top_viol$viol_cont, 2)) %>%
  addLegend("bottomright",
            pal = viol_pal,
...
# would love to plot by most common violation by utility: 
...
# Superfunpwsid_shortages# Superfund sites (NPL), risk management plan (RMP) proximity, 
...
real_basin <- colorFactor(
  palette = viridis::viridis(12),
  domain = basins$basin_name)
...
leaflet() %>%
  ...
  addPolygons(data = basins,
              ...
              # stroke = TRUE,
              # color = "black",
              color = ~real_basin(basin_name),
              weight = 1) %>%
  addPolygons(data = sw_imp_basins,
              fillOpacity = 0.8,
              stroke = TRUE,
              color = "black",
              fillColor = ~basin_pal(violation_flag),
              weight = 1,
              label = paste0("pwsid: ", sw_imp_basins$pwsid,
                "; cvi: ", sw_imp_basins$violation_flag)) %>%
  addLegend("bottomright",
            pal = basin_pal,
...
census <- demo$census %>% filter(east_tx_flag == "yes")
```
