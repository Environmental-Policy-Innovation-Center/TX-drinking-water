...
leaflet() %>%
  ...
  addPolygons(data = census_sf,
              ...
              weight = 1,
              label = paste0("pwsid: ", census_sf$pwsid,
                             "; poc: ", census_sf$estimate_poc_alone_per)) %>%
  addLegend("bottomright",
            pal = poc_pal,
            values =  census_sf$estimate_poc_alone_per,
            title = "% POC",
            opacity = 1)
...
mean(aff_analysis$total_water_year) 
# Mean just total water: 
# East TX: $511.2519
# TX: $545.5747
...
mhi_analysis <- demo$census %>%
  select(pwsid, east_tx_flag, estimate_mhi) %>%
  as.data.frame() %>%
  select(-geometry) %>%
...
mhi %>%
  ...
  relocate(quintiles, .after = mhi_bin)
# lower 20% = $23,600 - $43,700 inclusive  

# checking out characteristics of this group: 
lowest_quintile_mhi <- demo$census %>%
  filter(estimate_mhi <= 43700) %>% 
  filter(east_tx_flag == "yes")
# there are 515, 145 of which are in east tx
...
leaflet() %>%
  addProviderTiles(providers$CartoDB.VoyagerNoLabels, group = "Toner Lite") %>%
  addPolygons(data = mhi %>% st_as_sf() %>% st_transform('+proj=longlat +datum=WGS84'),
              fillOpacity = 0.9,
              color = ~mhi_pal_mini(estimate_mhi),
              weight = 1,
              label = paste0("pwsid: ", mhi$pwsid)) %>%
  addPolygons(data = lowest_quintile_mhi%>% st_transform('+proj=longlat +datum=WGS84'),
              fillOpacity = 0.9,
              color = "red",
              weight = 1) %>%
  addLegend("bottomright", 
...
# How does HCF relate to LQI? 
lqi_hcf <- aff %>%
  left_join(keys$analysis_keys, "pwsid") %>%
  filter(east_tx_flag == "yes")

ggplot(lqi_hcf, aes(x = estimate_mhi, y = total_water_year + total_sewer_year)) + 
  geom_point(aes(color = estimate_poc_alone_per), size = 5, 
             alpha = 0.8) + 
  # scale_color_continuous(name = "HCF") + 
  scale_color_continuous(name = "% POC") + 
  theme_minimal() + 
  labs(x = "Annual Median Household Income ($)", 
       y = "Annual Total Household Water Cost ($)") + 
  geom_vline(xintercept = 43700, lty = "dashed")
# Makes sense - they should be related since HCF is calculated using MHI
...
unemp_pal <- colorNumeric(
  palette = viridis::viridis(9),
  domain = census_sf$estimate_laborforce_unemployed_per)
...
leaflet() %>%
  ...
  addPolygons(data = census_sf,
              ...
              label = paste0("pwsid: ", census_sf$pwsid, 
                             "; % unemp: ", 
                             census_sf$estimate_laborforce_unemployed_per)) %>%
  addLegend("bottomright",
            pal = unemp_pal,
            values = census_sf$estimate_laborforce_unemployed_per,
            title = "% Unemployment",
            opacity = 1) 
...
interpolate <- areal::aw_interpolate(census_sf, tid="pwsid", 
                                     ...
                                     weight = "sum",
                                     output="sf", 
                                     intensive="percentage_of_tract_that_is_disadvantaged_by_area")


# using weighted mean to calculate mean % disadvantaged for pwsids
pwsid_disadv <- interpolate %>%
  group_by(pwsid) %>%
  summarize(per_disadv_mean = mean(percentage_of_tract_that_is_disadvantaged_by_area)) %>%
  as.data.frame() %>%
  select(-geometry)
...
cejst_analysis <- cejst_analysis %>%
  filter(east_tx_flag == "yes") %>%
  st_transform(., crs = st_crs(tract_geo))

# plotting by disadv status: 
cejst_pal <- colorNumeric(
  palette = viridis::viridis(9),
  domain = cejst_analysis$per_disadv_mean)

leaflet() %>%
  addProviderTiles(providers$CartoDB.VoyagerNoLabels, group = "Toner Lite") %>%
  addPolygons(data = cejst_analysis,
              fillOpacity = 0.9,
              color = ~cejst_pal(per_disadv_mean),
              weight = 1,
              label = paste0("pwsid: ", cejst_analysis$pwsid, 
                             "; % disadv: ", cejst_analysis$per_disadv_mean)) %>%
  addLegend("bottomright",
            pal = cejst_pal,
            values = cejst_analysis$per_disadv_mean,
            title = "% CEJST Disadv",
            opacity = 1) 

# box plot to investigate each bracket: 
dac_TX <- cejst_analysis %>%
  select(pwsid, per_disadv_mean) %>%
  as.data.frame() %>%
  select(-geometry) %>%
  mutate(flag = "TX")
mean(dac_TX$per_disadv_mean, na.rm = T)

dac_eTX <- cejst_analysis %>%
  select(pwsid, east_tx_flag, per_disadv_mean) %>%
  as.data.frame() %>%
  select(-geometry) %>%
  filter(east_tx_flag == "yes") %>%
  select(-east_tx_flag) %>%
  mutate(flag = "East TX")
mean(dac_eTX$per_disadv_mean, na.rm = T)
...
ggplot(all_dac, aes(x = flag,  
  ...
  scale_fill_discrete(name = "") +
  theme_minimal()  + 
  labs(x = "", y = "Percent of Service Area Disadv by CEJST") 

# looking at data by quantile of disadv status: 
cejst_analysis %>%
  as.data.frame() %>%
  select(-geometry) %>%
  mutate(per_disadv_mean = as.numeric(per_disadv_mean)) %>%
  mutate(dac_bin = cut(per_disadv_mean, 
                       breaks = c(unique(quantile(per_disadv_mean, 
...
# what does this look like in the form of a graph?
ggplot(cejst_analysis, aes(x = per_disadv_mean, 
                           y = estimate_mhi, color = healthbased_violations_5yr)) + 
  geom_point() + 
  theme_minimal() + 
  labs(x = "% of SAB that is Disadvantaged", y = "Median Household Income") + 
  scale_color_continuous(name = "Heath-based Violations (5yr)") + 
  theme(legend.position = "bottom")

# are there any clear correlations? 
all_cor <- cor(cejst_analysis %>% 
                 as.data.frame() %>%
                 select(-east_tx_flag) %>%
...
# can we link to which vulnerabilities are being exceeded?
# Just grabbing counties in East TX
east_tx <- read.csv("./data/raw/east_TX_counties.csv") %>%
  select(-X) %>%
  mutate(county_tidy = paste0(county_name, " County"))

cejst_raw <- aws.s3::s3read_using(read.csv, 
                                  object = "s3://tech-team-data/ej-data/cejest-v1/1.0-communities.csv") %>%
  janitor::clean_names() %>%
  filter(state_territory == "Texas") %>%
  filter(county_name %in% east_tx$county_tidy)

# grabbing mean percentile for each county: 
percentiles <- cejst_raw[, endsWith(names(cejst_raw), '_percentile')]
id_cols <- cejst_raw[,c("census_tract_2010_id", "county_name")]
cejst_percentiles <- cbind(id_cols, percentiles)
county_percentiles <- cejst_percentiles %>%
  group_by(county_name) %>%
  summarise(across(!c("census_tract_2010_id"), ~mean(.x, na.rm = T)))

etx_percentiles <- county_percentiles %>%
  summarise(across(!c("county_name"), ~mean(.x, na.rm = T))) %>%
  pivot_longer(1:31)
```

## Investigating stats for cws in TX and comparisons across different scales:
```{r}
...
# summaries for those served by community water systems: 
summary(census)
summary(east_tx_cws)
...
################################################################################
# How does this compare to all of East Texas?
################################################################################
...
summary(etx_wide_per)
sum(etx_wide$estimate_total_pop)
```

## Where are POC located at the tract level?
```{r}
etx_wide_sf <- etx_wide %>%
  mutate(percent_poc = (100*(estimate_total_pop - estimate_white_alone)/estimate_total_pop))

poc_pal <- colorNumeric(
  palette = viridis::viridis(9),
  domain = etx_wide_sf$percent_poc)

leaflet() %>%
    addPolygons(data = etx_wide_sf,
              fillOpacity = 0.3,
              color = ~poc_pal(percent_poc),
              weight = 1,
              label = paste0("% poc: ", etx_wide_sf$percent_poc)) %>%
  addProviderTiles(providers$CartoDB.VoyagerLabels, group = "Toner Lite") %>%
  addLegend("bottomright",
            pal = poc_pal,
            values = etx_wide_sf$percent_poc,
            title = "% POC",
            opacity = 1) 
```
...
# correlation plot across all metrics: 
all_data <- census %>%
  as.data.frame() %>%
  left_join(keys$analysis_keys) %>%
  # focusing on East TX for now: 
  filter(east_tx_flag == "yes")

all_cor <- cor(all_data %>% 
                 select(c(healthbased_violations_5yr,
                          paperwork_violations_5yr, 
                          healthbased_violations_10yr,
                          paperwork_violations_10yr,
...
name_changes <- c("Health Viols, 5yr", "Non-Health Viols, 5yr", 
                      "Health Viols, 10yr",  "Non-Helath Viols, 10yr", 
                       "Area (mi)", "Population Density", 
                       "Service Connections", 
                       "MHI", "% Households in Poverty", 
                       "% Unemployment", "% No Schooling", 
                       "% Highschool", "% Only Speak English",
