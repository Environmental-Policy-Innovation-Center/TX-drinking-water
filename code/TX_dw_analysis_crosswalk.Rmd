...
## Investigating output:
```{r}
complete_crosswalk_sf <- complete_crosswalk %>%
    st_transform('+proj=longlat +datum=WGS84')

pop_pal <- colorNumeric(
  palette = viridis::viridis(9),
  domain = complete_crosswalk_sf$estimate_total_pop,
  na.color = "red")

leaflet() %>%
  addProviderTiles(providers$CartoDB.VoyagerNoLabels,
                   group = "Toner Lite") %>%
  addPolygons(data = complete_crosswalk_sf,
              fillOpacity = 0.3,
              stroke = TRUE,
              # color = "black",
              color = ~pop_pal(estimate_total_pop),
              weight = 1,
              label = paste0("pwsid: ", complete_crosswalk_sf$pwsid,
                             "; mhi: ", round(complete_crosswalk_sf$estimate_mhi, 2), 
                             "; pop: ", round(complete_crosswalk_sf$estimate_total_pop, 2)))
...
complete_crosswalk %>%
  as.data.frame() %>%
  select(-geometry) %>%
  group_by(tier_crosswalk) %>%
  summarize(n())
```


## Scaling final function up to all of TX and all census vars: 
```{r}
################################################################################
## Data prep: 
################################################################################
...
# # they're all missing from the crosswalk :-(
# merge(pwsid_cross, missing, by = "pwsid")
# merge(as.data.frame(pw_final), missing, by = "pwsid")
...
final_sab_sf <- final_sab %>%
  st_transform("+proj=longlat +datum=WGS84")

pop_pal <- colorNumeric(
  palette = viridis::viridis(9),
  domain = final_sab_sf$estimate_mhi,
  na.color = "grey")

leaflet() %>%
  addProviderTiles(providers$CartoDB.VoyagerNoLabels,
                   group = "Toner Lite") %>%
  addPolygons(data = final_sab_sf,
              fillOpacity = 1,
              stroke = TRUE,
              # color = "black",
              color = ~pop_pal(estimate_mhi),
              weight = 1,
              label = paste0("pwsid: ", final_sab_sf$pwsid,
                             "; mhi: ", round(final_sab_sf$estimate_mhi, 2), 
                             "; pop: ", round(final_sab_sf$estimate_total_pop, 2))) %>%
  addLegend("bottomright",
            pal = pop_pal,
            values = final_sab_sf$estimate_mhi,
            title = "MHI",
            opacity = 1)
