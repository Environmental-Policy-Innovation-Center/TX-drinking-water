...
mhi_pal <- colorNumeric(
  palette = viridis::mako(9),
  domain = pwsid_mhi$estimate_mhi, 
  na.color = "red")

leaflet() %>%
  addProviderTiles(providers$CartoDB.VoyagerNoLabels, 
                   group = "Toner Lite") %>%
  addPolygons(data = pwsid_mhi,
              fillOpacity = 0.6,
              stroke = TRUE,
              # color = "black",
              color = ~mhi_pal(estimate_mhi),
              weight = 1,
              label = paste0("pwsid: ", pwsid_mhi$pwsid,
                             "; mhi: ", round(pwsid_mhi$estimate_mhi, 2)))

# some MHI are still missing 
...
# ar_validate(source = tx_wide_mini_pop, target = sab_pw, 
#             varList = "estimate_total_pop", 
#             verbose = T)

aw_inter_pop <- areal::aw_interpolate(sab_pw, tid = "pwsid", 
                      source = tx_wide_mini_pop, 
                      sid= "GEOID", 
                      weight = "total",
                      output="sf", 
                      extensive = c("estimate_total_pop", 
...
aw_inter_pwsid <- aw_inter_pop %>%
  filter(pwsid %in% unique(na_pw$pwsid)) %>%
...
# doing mhi using spatially intensive approach: 
tx_wide_mini_aw <- tx_wide_mhi %>% 
  st_transform(., crs = "ESRI:102296") %>%
  select(GEOID, estimate_mhi, mhi_percent_uni)

aw_inter_mhi <- areal::aw_interpolate(sab_pw, tid = "pwsid", 
                                      source = tx_wide_mini_aw, 
                                      sid= "GEOID", 
                                      weight = "sum",
                                      output="sf", 
                                      intensive = c("mhi_percent_uni"))
# putting back in context from universe: 
aw_inter_pwsid_mhi <- aw_inter_mhi %>%
  filter(pwsid %in% unique(na_pw$pwsid)) %>%
  mutate(estimate_mhi = mhi_percent_uni*sum(tx_wide_mini_aw$estimate_mhi, na.rm = T)) %>%
  as.data.frame() %>%
  select(-geometry) %>%
  select(pwsid, mhi_percent_uni, estimate_mhi)

# recombining with NA datastet: 
aw_inter_fill <- aw_inter_pwsid %>%
  left_join(aw_inter_pwsid_mhi)

# recombining with entire dataset: 
head(all_census)
aw_inter_fill_mini <- aw_inter_fill %>%
  select(pwsid, estimate_total_pop, estimate_white_alone, 
         mhi_percent_uni, estimate_mhi) %>%
