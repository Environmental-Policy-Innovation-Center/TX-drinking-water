...
## Testing method on full dataset for TX - a small test case: 
```{r}
###############################################################################
...
# interpoltating - need to use summed weights when using the spatially 
# intensive method: 
...
tx_inter <- areal::aw_interpolate(sab, tid = "pwsid", 
                                  ...
                                  # some light reorg to make it easier to 
...
# interpolating for mhi - since this is spatially intensive and requires 
# summed weights: 
tx_wide_mhi <- tx_wide %>%
  mutate(mhi_percent_uni = estimate_mhi / sum(tx_wide$estimate_mhi, na.rm = T))%>%
  st_transform(., crs = "ESRI:102296") %>%  
...
tx_inter_clean_per <- tx_inter %>%
  ...
  # adding median household income: 
  left_join(pwsid_mhi) %>%
  # leaving out columns that shouldn't be divided:
  mutate_at(vars(!c("estimate_total_pop", "area_miles", 
                    "estimate_mhi", "pwsid", "population_served_count", 
                    "estimate_laborforce", "estimate_laborforce_unemployed", 
                    "estimate_hh_total", "estimate_hh_below_pov", "sdwis_flag")),
...
sab_census <- merge(tx_inter, tx_inter_clean_per, by.x = "pwsid", 
                    by.y = "pwsid_per") %>%
  select(-c(estimate_total_pop_per, estimate_hh_total_per)) %>%
  rename(estimate_mhi = estimate_mhi_per)

# mutliplying SDWIS populations by percentages found through areal interpolation 
...
tx_sdwis_fix <- sab_census %>%
  ...
  as.data.frame() %>%
  select(-c(estimate_mhi, estimate_poc_alone_per, geometry)) %>%
