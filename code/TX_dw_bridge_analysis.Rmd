...
# 7.0 Can we fold in HUC-level information for CWS in TX?
```{r}
...
# all_hucs <- merge(well_pwsid_hucs, intake_pwsid_hucs, by = "pwsid", all = T)
# 
# 
# # filtering for TX: 
# tx_huc_info <- all_hucs %>% 
#   filter(pwsid %in% tx_xwalk$pwsid)
# test <- merge(tx_huc_info, app_data_ws_info, by = "pwsid", all.y = T)
```
