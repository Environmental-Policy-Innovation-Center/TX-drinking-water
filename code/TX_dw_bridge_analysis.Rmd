...
health_viol <- tx_viol_rulecode %>%
  ...
  summarize(total = length(unique(violation_id)))
...
pwsid_viol_summary <- tx_viol_rulecode %>%
  mutate(start_date = as.Date(viol_first_reported_date, 
                              tryFormats = c("%m/%d/%Y")), 
         start_year = year(start_date)) %>%
  filter(is_health_based_ind == "Y") %>%
  group_by(pwsid, start_year) %>%
  summarize(total_health_viols = length(unique(violation_id))) %>%
