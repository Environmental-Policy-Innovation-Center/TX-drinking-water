...
# what does the distribution of needs look like?
...
# grabbing enforcement actions from SDWIS: 
# NOTE - this is a summary table and it looks like the models haven't 
# been run since 2018.. 
# source("./code/functions/get_SDWIS_enforcement_summary.R")
# sdwis_enf <- get_SDWIS_enforcement(pwsids = demo$census$pwsid, state = "TX")

# grabbing summary enforcement data and merging with all pwsids in our study: 
# enf_summary <- sdwis_enf %>%
#   group_by(pwsid, enf_year) %>%
#   summarize(num_enf = length(unique(enfdate))) 
# 
# en_summary_all <- merge(enf_summary, keys$analysis_keys, by = "pwsid", 
#                         all.y = T) %>%
#   mutate(num_enf = replace_na(num_enf, 0))
# 
# # plot to see distribution: 
# ggplot(en_summary_all, aes(x = num_enf)) +
#   geom_histogram() + 
#   facet_wrap(~enf_year, scales = "free")
# 
# mean(en_summary_all$num_enf)
