...
census <- tidycensus::get_acs(
  ...
  # also wanted to grab stats for neighboring states since the crosswalk
  # depends on them for SABs close to the border: 
  state = c("TX", "AR", "OK", "NM", 
            "LA"), 
  year = 2021,
  geometry = TRUE
...
sab_census_crosswalked <- sab_cross_census_weight %>%
  group_by(pwsid) %>% 
    summarize(across(estimate_total_pop:estimate_hh_below_pov, ~round(sum(.), 0)), 
              estimate_mhi = weighted.mean(estimate_mhi, tract_parcel_weight, na.rm = TRUE), 
              across(estimate_laborforce:estimate_other_lang, ~round(sum(.), 0))) 
...
# TODO: ask funders which pwsids they care about - some were captured in the 
# intersection but others are fairly close to the border 
...
sab_census_flag <- sab_census %>%
  mutate(east_tx_flag = case_when(pwsid %in% east_tx_pwsids ~ "yes",
                                  TRUE ~ "no")
         ) %>%
