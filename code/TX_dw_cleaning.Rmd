...
diag_log <- data.frame(diagnostic = "") %>%
  mutate(zero_sdwis_pop = sum(TX_pwsid_census_tidy$population_served_count == 0, 
                              na.rm = TRUE), 
         na_sdwis_pop = sum(is.na(TX_pwsid_census_tidy$population_served_count)), 
         # where there are empty geometries - in the original TX_sab dataset 
         empty_geoms = sum(sf::st_is_empty(TX_pwsid_census_tidy)), 
         na_census_data = sum(is.na(TX_pwsid_census_tidy$total_pop)), 
