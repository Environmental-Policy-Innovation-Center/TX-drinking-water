...
library(aws.s3)
library(janitor)
library(rvest) # only used in webpage scraping 
```
...
# census data - grabbing block stats for now 
...
# PPL data 
TX_PPL <- read_csv(get_object(object = "clean_data/srf_project_priority_lists/web_ppl_combined_clean_v1-1.csv", 
...
# TODO: Note that the crosswalk method will change based on our validation 
# analysis !! 

# TODO: Flag for rural or non-rural here? 

# grabbing counties: 
counties <- unlist(strsplit(TX_census$NAME, split = ","))
TX_census$counties <- trimws(counties[seq(4, length(counties), by = 5)])

# crosswalking TX sabs with census demographic info:
TX_census_wide <- pivot_wider(TX_census, 
...
# merging this back with sab dataset for mapping: 
TX_pwsid_census_tidy <- merge(TX_sab, pwsid_census_per, all = T)

## Merging SDWIS, sabs, census, and srf ######################################## 
## NOTE: add some more SDWIS stats at this step (i.e, year, decade, length of 
# violation, etc.)
...
# contains pwsid-level demographic information, SDWIS violations, and srf data
```
...
# filtering census data for just east Texas counties:
eastTX <- TX_census_wide %>%
  filter(counties %in% paste(east_TX_counties$county_name, "County"))
